...
from Cube_Skill.Cube_Application import on_processing_error
...
class VoiceHandler(ResponseBuilder):
    ...
    def route_request(self, request_json, metadata=None):
        ...
        elif not request.is_intent() and request.intent_name() not in self._handlers['IntentRequest']:
            ''' If intent doesn't exist'''
            message = Error()
            handler_fn = on_processing_error(message)
